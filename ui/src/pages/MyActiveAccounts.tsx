...
export const MyActiveAccountsPage: React.FC<MyActiveAccountsPageProps> = (
  ...
  const assetRows = contracts.map((contract) => (
    <AssetAccountRow
      ...
      price={contract.payload.bondData.price}
      interestRate= {contract.payload.bondData.interestRate}
      amountIssued = {contract.payload.bondData.amountIssued}
      duration = {contract.payload.bondData.duration}
      bondIssuer = {contract.payload.bondData.bondIssuer}
    />
  ));
